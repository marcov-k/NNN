...
extern "C"
{
	...
	__declspec(dllexport) void* tensor_create_scalar(float value, const int* dims, int rank, bool requires_grad);
	...
	__declspec(dllexport) float* tensor_data_ptr(void* handle);

	// Returns the value at the given linear index in the given tensor.
	__declspec(dllexport) float tensor_get_at(void* handle, int index);

	// Sets the value at the given linear index in the given tensor.
	__declspec(dllexport) void tensor_set_at(void* handle, float value, int index);

	// Returns the value at the given indices in the given tensor.
	__declspec(dllexport) float tensor_get_at_spatial(void* handle, const int* indices, int rank);

	// Sets the value at the given indices in the given tensor.
	__declspec(dllexport) void tensor_set_at_spatial(void* handle, float value, const int* indices, int rank);

	// Returns a pointer to the start of gradient vector of the given tensor.
	__declspec(dllexport) float* tensor_grad_ptr(void* handle);
	...
	__declspec(dllexport) void* tensor_add_scalar(void* handle_a, float b);
	...
	__declspec(dllexport) void* tensor_sub_scalar(void* handle_a, float b);

	// Subtracts the given scalar and tensor -> (a (S) - b (T))
	__declspec(dllexport) void* tensor_sub_scalar_left(float a, void* handle_b);
	...
	__declspec(dllexport) void* tensor_mul_scalar(void* handle_a, float b);
	...
	__declspec(dllexport) void* tensor_div_scalar(void* handle_a, float b);

	// Divides the given scalar and tensor -> (a (S) / b (T))
	__declspec(dllexport) void* tensor_div_scalar_left(float a, void* handle_b);
	...
	__declspec(dllexport) void* tensor_pow_scalar(void* handle_a, float exp);

	// Raises the given scalar to the given exponent tensor -> (a (S) ^ exp (T))
	__declspec(dllexport) void* tensor_pow_scalar_left(float a, void* handle_exp);
	...
	__declspec(dllexport) void* tensor_log_scalar(void* handle_arg, float log_base);

	// Computes the logarithm with the given tensor base of the given scalar -> (log_baseT(arg (S))
	__declspec(dllexport) void* tensor_log_scalar_left(float arg, void* handle_log_base);
	...
	__declspec(dllexport) void* tensor_get_dense_dropout_mask(const int* dims, int dims_length, float dropout);

	// Initializes a new mask applying spatial dropout to a tensor with the given dimensions.
	__declspec(dllexport) void* tensor_get_spatial_dropout_mask(const int* dims, int dims_length, float dropout);
	...
	__declspec(dllexport) void* tensor_leaky_relu(void* handle, float tau);
	...
	__declspec(dllexport) void* tensor_huber(void* handle_t, void* handle_target, float delta);
	...
	__declspec(dllexport) void models_clip_gradients(void** handles, int para_count, float max_norm);

	// Applies a soft update to the given target model's parameters based on the given agent model's parameters.
	__declspec(dllexport) void models_soft_update(void** handles_agent, void** handles_target, int para_count, float tau,
		float one_minus_tau);
}