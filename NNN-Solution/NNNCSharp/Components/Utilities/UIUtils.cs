...
namespace NNNCSharp.Components.Utilities
{
    ...
    public static class UIUtils
    {
        ...
        {
            if (GetInput("Replay past episodes? y/n", new() { userInputs[UserInput.Yes], userInputs[UserInput.No] }) == userInputs[UserInput.Yes])
            {
                ...
                while (true)
                {
                    ...
                    if (GetInput("View another episode? y/n", new() { userInputs[UserInput.Yes], userInputs[UserInput.No] }) == userInputs[UserInput.No]) break;
                }
            }
        }
        ...
        public static void SaveLoop(Model model)
        {
            ...
            while (true)
            {
                ...
                {
                    if (GetInput($"File with name \"{fileName}\" already exists. Overwrite existing file? y/n", new() { userInputs[UserInput.Yes], userInputs[UserInput.No] })
        ...
        static void Save(Model model, string fileName)
        {
            ...
            if (GetInput("Include a short description of the model? y/n", new() { userInputs[UserInput.Yes], userInputs[UserInput.No] }) == userInputs[UserInput.Yes])
            {
