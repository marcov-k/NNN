...
namespace NNNCSharp.Components.DQNEnvironments
{
    ...
    public class MovementGrid2D : DQNEnvironment
    {
        ...
        public override Tensor StateFormat => new(new int[] { 1, 4 }); // encodes agent's and target's grid positions
        ...
        /// </summary>
        readonly Tensor State = new(new int[] { 4 }); // agent x, agent y, target x, target y
        /// <summary>
        ...
        {
            Bounds = new int[] { xMin, xMax, yMin, yMax };
            XRange = xMax - xMin;
            YRange = yMax - yMin;
            MaxSteps = maxSteps;
        ...
        {
            Tensor normalized = new(new int[] { 4 });
