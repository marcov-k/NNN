...
namespace NNNCSharp.Components.DQNEnvironments
{
    ...
    public class Snake : DQNEnvironment
    {
        ...
        public override Tensor StateFormat => new(new int[] { 1, GridDims.Y, GridDims.X, 8 }); // 20 x 20 grid with one-hot encoding for contents of each cell and the direction each cell's content will move in
        ...
        {
            HashSet<Int2> visited = new(); // HashSet of unique visited positions
            Queue<Int2> queue = new(); // Queue of positions to visit
        ...
        List<Int2> GetNeighbors(Int2 pos)
        {
            ...
            List<Int2> neighbors = new() { new(pos.X - 1, pos.Y), new(pos.X, pos.Y - 1), new(pos.X + 1, pos.Y), new(pos.X, pos.Y + 1) };
            List<Int2> validNeighbors = new();

            // Remove all neighbors with invalid positions
            foreach (var neighbor in neighbors)
            {
                if (ValidPosition(neighbor)) validNeighbors.Add(neighbor);
            }
        ...
        {
            Tensor state = new(new int[] { GridDims.Y, GridDims.X });
